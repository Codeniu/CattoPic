...
export async function imagesHandler(
  ...
  try {
    ...
    const limit = clampInt(
      parseNumber(url.searchParams.get('limit'), 12),
      1,
      MAX_IMAGES_PAGE_SIZE
    )
    ...
    const format =
      validateImageListFormat(url.searchParams.get('format')) || 'all'
    ...
    const { images, total } = await metadata.getImages({
      page,
      limit,
      tag,
      orientation,
    ...
    if (body.tags !== undefined) {
      if (body.tags === null) {
        ...
        const normalized = body.tags
          .map((tag: unknown) =>
            sanitizeTagName(typeof tag === 'string' ? tag : String(tag))
          )
