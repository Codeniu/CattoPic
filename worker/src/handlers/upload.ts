...
export async function uploadSingleHandler(
  ...
  try {
    ...
    const expiryMinutes = parseNumber(
      formData.get('expiryMinutes') as string | null,
      0
    )
    ...
    const compression = c.env.IMAGES
      ? new CompressionService(c.env.IMAGES)
      : null
    ...
    const originalUploadPromise = storage.upload(
      paths.original,
      arrayBuffer,
      contentType
    )
    ...
    if (shouldSkipProcessing) {
      ...
      if (wantsWebp && compressionResult.webp) {
        ...
        uploadPromises.push(
          storage
            .upload(paths.webp, compressionResult.webp.data, 'image/webp')
            .then(() => {
      ...
      if (wantsAvif && compressionResult.avif) {
        ...
        uploadPromises.push(
          storage
            .upload(paths.avif, compressionResult.avif.data, 'image/avif')
            .then(() => {
    ...
    c.executionCtx.waitUntil(
      Promise.all([cache.invalidateImagesList(), cache.invalidateTagsList()])
    )

    return successResponse({ result })
  } catch (err) {
    console.error('Single upload error:', err)
    return errorResponse('Upload failed1')
  }
}
