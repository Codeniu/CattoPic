...
export async function renameTagHandler(
  c: Context<{ Bindings: Env }>
): Promise<Response> {
  try {
...
export async function deleteTagHandler(
  ...
  try {
    ...
    console.log(
      `[deleteTag] images matched: name=${name} count=${images.length}`
    )
    ...
    console.log(
      `[deleteTag] D1 delete completed: name=${name} deletedImages=${deletedImages}`
    )
    ...
    if (images.length > 0) {
      ...
      for (const img of images) {
        const keys = Array.from(
          new Set(
            [img.paths.original, img.paths.webp, img.paths.avif].filter(
              isNonEmptyString
            )
          )
        )
      ...
      for (let i = 0; i < allKeysToDelete.length; i += chunkSize) {
        ...
        try {
          await storage.deleteMany(chunk)
        } catch (err) {
          console.error(
            `[deleteTag] R2 delete failed: name=${name} chunk=${i}-${
              i + chunk.length - 1
            }`,
            err
          )
      ...
      console.log(
        `[deleteTag] R2 delete completed: name=${name} files=${allKeysToDelete.length}`
      )
    }
...
export async function batchTagsHandler(
  c: Context<{ Bindings: Env }>
): Promise<Response> {
  try {
    const body = await c.req.json()
    ...
    const sanitizedAddTags = (addTags || [])
      .map(sanitizeTagName)
      .filter(Boolean)
    const sanitizedRemoveTags = (removeTags || [])
      .map(sanitizeTagName)
      .filter(Boolean)
    ...
    const metadata = new MetadataService(c.env.DB)
    const updatedCount = await metadata.batchUpdateTags(
      imageIds,
      sanitizedAddTags,
      sanitizedRemoveTags
    )
